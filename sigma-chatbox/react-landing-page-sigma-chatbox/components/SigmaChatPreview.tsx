...
const SigmaChatPreview: React.FC = () => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Chào bạn, mình là trợ lý AI của Sigma Shop! Bạn cần hỗ trợ gì hoặc có thể chọn một trong các chủ đề dưới đây nhé.",
    },
  ]);
  ...
  const handleSimulatedSubmit = (text?: string) => {
    ...
    setTimeout(() => {
      const responses: Record<string, string> = {
        "Chính sách bảo hành":
          "Chính sách bảo hành của Sigma là 1 đổi 1 trong vòng 12 tháng đối với lỗi nhà sản xuất bạn nhé! ✨",
        "Tra cứu đơn hàng":
          "Vui lòng nhập mã đơn hàng (ví dụ: SIG123) để mình kiểm tra giúp bạn.",
        "Khuyến mãi HOT":
          "Hiện tại đang có chương trình giảm giá 20% cho khách hàng mới đấy!",
        default:
          "Cảm ơn bạn đã quan tâm! Đây là bản demo giao diện của Sigma Chat Box.",
      };
  ...
  return (
    <div className="w-full max-w-[400px] h-[580px] bg-[#FDE8E9] rounded-[32px] shadow-2xl overflow-hidden border border-white/40 flex flex-col relative font-sans">
      ...
      <div className="bg-gradient-to-r from-[#FF4D5E] to-[#FF758C] p-4 flex items-center justify-between text-white">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm">
            <img
              src="https://api.dicebear.com/7.x/bottts/svg?seed=Sigma&backgroundColor=transparent"
              alt="Mascot"
              className="w-6 h-6"
            />
          </div>
      ...
      <div
        ref={scrollRef}
        className="flex-grow overflow-y-auto p-4 space-y-5 scroll-smooth no-scrollbar"
      >
        <div className="flex flex-col items-center py-4">
          <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-lg mb-2 border-2 border-pink-100 overflow-hidden">
            <img
              src="https://api.dicebear.com/7.x/bottts/svg?seed=Sigma&backgroundColor=b6e3f4"
              alt="Sigma"
            />
          </div>
          <h3 className="text-lg font-bold text-[#333]">
            Sigma
            <span className="bg-[#FF4D5E] text-white text-[10px] px-1 rounded ml-1">
              AI
            </span>
          </h3>
          <p className="text-center text-[12px] text-gray-500 font-medium px-4">
            <b>Sigma - Biết tuốt</b> hỗ trợ bạn tra cứu nhanh sản phẩm và đơn
            hàng.
          </p>
        </div>
          ...
          >
            {msg.role === "assistant" && (
              <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-sm mb-1 overflow-hidden">
                <img
                  src="https://api.dicebear.com/7.x/bottts/svg?seed=Sigma"
                  alt="AI"
                />
              </div>
            )}
